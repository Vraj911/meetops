...
function Upload() {
  ...
  return <div className="max-w-2xl mx-auto space-y-8 animate-fade-in"><div><h1 className="text-3xl font-semibold">Upload Meeting</h1><p className="text-muted-foreground mt-1">Add your meeting transcript or audio for AI processing</p></div>{
    /* Meeting Details */
  }<div className="space-y-4"><h2 className="text-lg font-medium">Meeting Details</h2><div className="grid grid-cols-2 gap-4"><div className="col-span-2 space-y-2"><label className="text-sm font-medium">Title</label><Input value={title} onChange={(e) => setTitle(e.target.value)} /></div><div className="space-y-2"><label className="text-sm font-medium">Date</label><Input type="date" value={date} onChange={(e) => setDate(e.target.value)} /></div><div className="space-y-2"><label className="text-sm font-medium">Time</label><Input type="time" value={time} onChange={(e) => setTime(e.target.value)} /></div></div></div>{
    /* Upload Method Toggle */
  }<div className="space-y-4"><h2 className="text-lg font-medium">Upload Method</h2><div className="flex gap-2"><Button variant={uploadMethod === "transcript" ? "default" : "outline"} onClick={() => setUploadMethod("transcript")} className="flex-1"><FileText className="h-4 w-4 mr-2" />Transcript
          </Button><Button variant={uploadMethod === "audio" ? "default" : "outline"} onClick={() => setUploadMethod("audio")} className="flex-1"><Music className="h-4 w-4 mr-2" />Audio
          </Button></div></div>{
    /* Dropzone */
  }<div
    onDragOver={(e) => {
