...
import { Sparkles, Save, ArrowRight, AlertTriangle } from "lucide-react";
...
function Review() {
  ...
  return <div className="animate-fade-in"><div className="flex items-center justify-between mb-6"><div><h1 className="text-3xl font-semibold">Review & Confirm</h1><p className="text-muted-foreground">{meeting?.title}</p></div><div className="flex gap-2"><Button variant="outline" onClick={() => setAiPanelOpen(!aiPanelOpen)}><Sparkles className="h-4 w-4 mr-2" />Refine with AI
          </Button><Button variant="outline"><Save className="h-4 w-4 mr-2" />Save Draft</Button><Button onClick={handleSync}><ArrowRight className="h-4 w-4 mr-2" />Prepare to Sync</Button></div></div><div className="grid grid-cols-1 lg:grid-cols-3 gap-6">{
    /* Summary Panel */
